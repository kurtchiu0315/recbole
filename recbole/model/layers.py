...
class FeedForward(nn.Module):
    ...
    def __init__(self, hidden_size, inner_size, hidden_dropout_prob, hidden_act, layer_norm_eps):
        ...
        self.dense_1 = nn.Linear(hidden_size, inner_size) # (hidden_size, inner_size = 4 * hidden_size)
        self.intermediate_act_fn = self.get_hidden_act(hidden_act)

        self.dense_2 = nn.Linear(inner_size, hidden_size) # (inner_size = 4 * hidden_size, hidden_size)
        self.LayerNorm = nn.LayerNorm(hidden_size, eps=layer_norm_eps)
...
class SingleFeedForward(nn.Module):
    ...
    Point-wise feed-forward layer is implemented by one dense layers.
    ...
    def __init__(self, hidden_size, hidden_dropout_prob, hidden_act, layer_norm_eps):
        super(SingleFeedForward, self).__init__()
        self.dense = nn.Linear(hidden_size, hidden_size) # (hidden_size, hidden_size)
        self.intermediate_act_fn = self.get_hidden_act(hidden_act)
        self.LayerNorm = nn.LayerNorm(hidden_size, eps=layer_norm_eps)
        self.dropout = nn.Dropout(hidden_dropout_prob)
...
class AugTransformerEncoder(nn.Module):
    r""" One TransformerEncoder consists of several TransformerLayers.
    ...
    def __init__(
        ...
        hidden_act='gelu',
        layer_norm_eps=1e-12,
        aug_layer_type='single',
        layer_drop_num=1,
        n_aug_layer=3,
        layer_drop_thres=-1
    ):

        super(AugTransformerEncoder, self).__init__()
        layer = TransformerLayer(
            n_heads, hidden_size, inner_size, hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps
        )
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(n_layers)])
        ...
        if layer_drop_thres >= 0:
            self.aug_layer = LayerDropProb(hidden_size,
                                           inner_size,
                                           hidden_dropout_prob,
                                           attn_dropout_prob,
                                           hidden_act,
            ...
            self.aug_layer = LayerDrop(hidden_size,
                                       inner_size,
                                       hidden_dropout_prob,
                                       hidden_act,
                                       layer_norm_eps,
...
class LayerDrop(nn.Module):
    def __init__(self, 
        ...
        self.aug_layer_type = aug_layer_type
        self.layer_drop_num = layer_drop_num
        self.num_aug_layers = n_aug_layer
        
        if self.aug_layer_type == 'single':
            
            single_layer = SingleFeedForward(hidden_size,
                                             hidden_dropout_prob, 
                                             hidden_act, 
                                             layer_norm_eps)
            self.aug_layers = nn.ModuleList([copy.deepcopy(single_layer) for _ in range(n_aug_layer)])
            
        elif self.aug_layer_type == 'inter':
            
            inter_layer = FeedForward(hidden_size, 
                                      intermediate_size, 
                                      hidden_dropout_prob, 
                                      hidden_act, 
                                      layer_norm_eps)
            self.aug_layers = nn.ModuleList([copy.deepcopy(inter_layer) for _ in range(n_aug_layer)])
    ...
    def forward(self, hidden_states, attention_mask, isTrain=False):
        ...
        if isTrain: # drop layers during training
            ...
            for idx, layer_module in enumerate(self.aug_layers):
                if idx not in drop_idx:
                    hidden_states = layer_module(hidden_states)
            
        else:
            
            for layer_module in self.aug_layers:
                hidden_states = layer_module(hidden_states)
...
class LayerDropProb(nn.Module):
    def __init__(self, 
        ...
        self.layer_drop_num = layer_drop_num
        self.num_aug_layers = n_aug_layer
        self.layer_drop_thres = layer_drop_thres
