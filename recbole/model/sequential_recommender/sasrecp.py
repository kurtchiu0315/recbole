...
class SASRecP(SequentialRecommender):
    ...
    def forwardCLS(self, item_seq, item_seq_len, condition, bidirectional=True):
        nitem_seq = torch.cat((torch.ones((item_seq.size(0), 1)).to(self.device), item_seq), dim=1)
        ...
        extended_attention_mask = self.get_attention_mask(nitem_seq, bidirectional=bidirectional)
