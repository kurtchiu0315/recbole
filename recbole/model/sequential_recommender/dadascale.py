...
class DADASCALE(CL4Rec):
  
    def __init__(self, config, dataset):
        super(DADASCALE, self).__init__(config, dataset)
...
class ScaleSelector(nn.Module):
    def __init__(self, aug_num, config):
        ...
        self.scale_trm_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(
                d_model=self.MAX_ITEM_LIST_LENGTH, 
                nhead=1 ,
                dim_feedforward=self.inner_size,
        ...
        self.fc_after_trm = nn.Sequential(
            nn.Dropout(self.hidden_dropout_prob),
            nn.Linear(self.MAX_ITEM_LIST_LENGTH, 1)
        )
    ...
    def forward(self, data):
        self.scale_trm_encoder.to(data.device)
        self.fc_after_trm.to(data.device)

        out = self.policy_trm_encoder(data * 1.0)

        # out: [B, aug_num, MAX_LEN]
        out = out.reshape(len(data), -1)
        out = self.fc_after_trm(out)
        # out: [B, aug_num]
