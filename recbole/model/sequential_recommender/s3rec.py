...
class S3Rec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.current_stage = 'pretrain' if self.train_stage == 'pretrain_finetune' else self.train_stage
        ...
        if self.loss_type == 'BPR':
            self.finetune_loss_fct = BPRLoss()
        elif self.loss_type == 'CE':
            self.finetune_loss_fct = nn.CrossEntropyLoss()
        else:
            raise NotImplementedError("Make sure 'loss_type' in ['BPR', 'CE']!")

        # parameters initialization
        assert self.current_stage in ['pretrain', 'finetune']
    ...
    def calculate_loss(self, interaction):
        ...
        # finetune
            ...
            if self.loss_type == 'BPR':
                ...
                loss =  self.finetune_loss_fct(pos_score, neg_score)
            else:  # self.loss_type = 'CE'
                test_item_emb = self.item_embedding.weight
                logits = torch.matmul(seq_output, test_item_emb.transpose(0, 1))
                loss =  self.finetune_loss_fct(logits, pos_items)
        return loss
