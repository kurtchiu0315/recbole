    ...
    def pretrain(self, train_data, verbose=True, show_progress=False):
        for epoch_idx in range(self.start_epoch, self.pretrain_epochs):
            ...
            if (epoch_idx + 1) % self.save_step == 0:
                self.save_pretrained_model(epoch_idx, self.saved_model_file)
                update_output = set_color('Saving current', 'blue') + ': %s' % self.saved_model_file
                if verbose:
                    self.logger.info(update_output)
...
class S3RecTrainer(PretrainTrainer):
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        elif self.model.train_stage == 'pretrain_finetune':
            self.pretrain(train_data, verbose, show_progress)
            self.model.current_stage = 'finetune'
            self.model.pre_model_path = self.saved_model_file
            self.model.load_pretrained_model()
            return super().fit(train_data, valid_data, verbose, saved, show_progress, callback_fn)
        else:
            raise ValueError("Please make sure that the 'train_stage' is 'pretrain' or 'finetune' or 'pretrain_finetune'!")
