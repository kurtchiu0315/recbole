...
def get_dataloader(config, phase):
    ...
    register_table = {
        ...
        'ICLRec': _get_CL_dataloader,
        'SRMA': _get_CL_dataloader,
        'ContraRec': _get_CL_dataloader,
        'DSSRec': _get_CL_dataloader,
...
def _get_CL_dataloader(config, phase):
    ...
    """
    if phase == 'train':
        if config['model'] in ['CL4Rec', 'ICLRec', 'ContraRec']:
            return CL4RecTrainDataLoader
        elif config['model'] == 'SRMA':
            return SRMATrainDataLoader
        ...
        elif config['model'] == 'DSSRec':
            return DSSRecTrainDataLoader
        elif config['model'] == 'EC4SRec':
